...
**Inputs.** `--input <out.txt>` (required); `--output <path>` (optional; default: `<input-base>_report.html` written alongside `--input`, e.g. `.../out.txt` → `.../out_report.html`); `--bin-dir <path>` (required, or resolved from env / global config, or auto-resolved to the latest install under `/opt/postgresql` when unset — existing binaries only, nothing is built); `--pg-gather-dir <path>` (required, or resolved from env / global config, or auto-discovered when unset — each `$PATH` entry is searched for one holding both `gather_schema.sql` and `gather_report.sql`, the first match is used and logged; the current working directory is deliberately **never** searched, because `report` is typically run from inside an extracted, untrusted support bundle and a bundle shipping planted gather scripts must not be silently executed); `--destroy-on-failure` / `-D` (optional). The command refuses by default rather than auto-downloading the gather scripts: missing inputs are errors, not prompts, so there is no `--force` flag. Auto-discovery only *finds scripts already on disk* — it never downloads them — so a fully-missing gather dir still errors with `EXIT_PG_GATHER_DIR_MISSING`. `--destroy-on-failure` is **not** a prompt-suppressor (there is no prompt): it controls failure cleanup, so it is a separate, explicitly-named flag rather than an overload of `--force`.

**Behavior.** The captured `out.txt` comes from someone else's system and is executed as a psql script, so it is treated as **untrusted input**: before anything is deployed or piped to psql, the file is pre-scanned and refused (with the offending line number, `EXIT_USAGE`) if it contains psql meta-commands, backquote substitution, or `COPY ... TO|FROM PROGRAM` constructs a genuine pg_gather capture never produces; COPY data rows are exempt (they may contain anything). Internally calls §6.1, §6.5 (psql piping `gather_schema.sql`, then `\copy out.txt`, then `gather_report.sql > report.html`), §6.3. The throwaway sandbox uses a temp directory inside the sandbox root; on success, it is destroyed; on failure, it is left in place for debugging and the temp path is printed — unless `--destroy-on-failure` / `-D` is set, in which case it is destroyed on failure too (falling back to leaving it in place, with a warning, if that cleanup itself fails).
