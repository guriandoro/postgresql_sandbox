...
```sh
# Primary
pg_sandbox deploy -s ~/postgresql-sandboxes/primary

# Async standby attached to it
pg_sandbox deploy -s ~/postgresql-sandboxes/standby1 \
...
pg_sandbox deploy -s ~/postgresql-sandboxes/standby2 \
    --replicate-from primary --slot primary_standby2_slot

# Inspect (text output — `status --json` currently emits a stub payload).
pg_sandbox status -s ~/postgresql-sandboxes/primary

# Promote standby1 if primary dies
pg_sandbox promote -s ~/postgresql-sandboxes/standby1
```
...
```sh
# Publisher
pg_sandbox deploy -s ~/postgresql-sandboxes/pub
pg_sandbox publish -s ~/postgresql-sandboxes/pub --pub-name my_pub --all-tables

# Subscriber (creates a fresh sandbox already subscribed)
pg_sandbox deploy -s ~/postgresql-sandboxes/sub \
...
pg_sandbox cluster deploy -s ~/postgresql-sandboxes/mycluster -N 2 --sync-count 1

# Cluster-wide consolidated status
pg_sandbox cluster status -s ~/postgresql-sandboxes/mycluster

# Tear down everything in one shot
pg_sandbox cluster destroy -s ~/postgresql-sandboxes/mycluster --force
```
